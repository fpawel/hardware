...
func (x T2500) Start(log comm.Logger, ctx context.Context) error {
	_, err := getResponse(log, ctx, "старт", comm.T(x), "01WRD,01,0102,0001")
	return err
}

func (x T2500) Stop(log comm.Logger, ctx context.Context) error {
	_, err := getResponse(log, ctx, "стоп", comm.T(x), "01WRD,01,0102,0004")
	return err
}
...
func (x T2500) Setup(log comm.Logger, ctx context.Context, value float64) error {
	...
	_, err := getResponse(log, ctx, fmt.Sprintf("уставка %v⁰C", value), comm.T(x), s)
	return err
}

func (x T2500) Read(log comm.Logger, ctx context.Context) (float64, error) {
	return getResponse(log, ctx, "запрос температуры", comm.T(x), "01RRD,02,0001,0002")
}
